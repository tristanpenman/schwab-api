...
class Schwab(SessionManager):
    ...
    def get_session_timeout(self):
        return self.get_user_preference("sessionTimeout")

    def get_user_preference(self, storage_key):
        data = {
            "storageKey": storage_key
        }

        full_url = urllib.parse.urljoin(urls.user_preferences(), '?' + urllib.parse.urlencode(data))
        self.update_token(token_type='update')
        r = requests.get(full_url, headers=self.headers)
        if r.status_code != 200:
            return [r.text], False
